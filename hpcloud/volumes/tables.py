...
class DeleteVolume(tables.DeleteAction):
    ...
    def allowed(self, request, volume=None):
        if volume:
            if "Delete Volume" in request.session['user_policies'].get(request.user.hpname):
                return True  and volume.status in DELETABLE_STATES
            else:
                return False
        else:
            return False
...
class CreateVolume(tables.LinkAction):
    ...
    def allowed(self, request, volume=None):
        if "Create Volume" in request.session['user_policies'].get(request.user.hpname):
            return True
        else:
            return False
...
class EditAttachments(tables.LinkAction):
    ...
    url = "horizon:hpcloud:volumes:attach"
    classes = ("ajax-modal", "btn-edit")

    def allowed(self, request, volume=None):
        if "Attach Volume" in request.session['user_policies'].get(request.user.hpname):
            return True and volume.status in ("available", "in-use")
        else:
            return False
...
class CreateSnapshot(tables.LinkAction):
    ...
    url = "horizon:hpcloud:volumes:create_snapshot"
    classes = ("ajax-modal", "btn-camera")

    def allowed(self, request, volume=None):
        if "Create Snapshot" in request.session['user_policies'].get(request.user.hpname):
            return True and  volume.status in ("available",)
        else:
            return False
...
def get_attachment_name(request, attachment):
    ...
    try:
        url = reverse("horizon:hpcloud:instances:detail", args=(server_id,))
...
class DetachVolume(tables.BatchAction):
    ...
    def allowed(self, request, volume=None):
        if "Dettach Volume" in request.session['user_policies'].get(request.user.hpname):
            return True
        else:
            return False
    ...
    def get_success_url(self, request):
        return reverse('horizon:hpcloud:volumes:index')
