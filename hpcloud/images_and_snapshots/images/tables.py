...
class DeleteImage(tables.DeleteAction):
    ...
    def allowed(self, request, image=None):
        ...
        if image and image.protected:
            return False
        if image:
            return image.owner == request.user.hp_attr.tenant_id
...
class CreateImage(tables.LinkAction):
    ...
    url = "horizon:hpcloud:images_and_snapshots:images:create"
...
class EditImage(tables.LinkAction):
    name = "edit"
    verbose_name = _("Edit")
    url = "horizon:hpcloud:images_and_snapshots:images:update"
    classes = ("ajax-modal", "btn-edit")

    def allowed(self, request, image=None):
        if image:
            return image.status in ("active",) and \
                image.owner == request.user.hp_attr.tenant_id
...
class OwnerFilter(tables.FixedFilterAction):
    ...
    def categorize(self, table, images):
        user_tenant_id = table.request.user.hp_attr.tenant_id
...
class UpdateRow(tables.Row):
    ...
    def load_cells(self, image=None):
        ...
        image = self.datum
        my_tenant_id = self.table.request.user.hp_attr.tenant_id
...
class ImagesTable(tables.DataTable):
    ...
    name = tables.Column(get_image_name,
                         link=("horizon:hpcloud:images_and_snapshots:"
                               "images:detail"),
