...
class DeleteKeyPairs(tables.DeleteAction):
    ...
    def allowed(self, request, kp=None):
        if "Delete KP" in request.session['user_policies'].get(request.user.openstackname):
            return True
        else:
            return False
...
class CreateKeyPair(tables.LinkAction):
    ...
    def allowed(self, request, keypair=None):
        ...
        if "Create KP" in request.session['user_policies'].get(request.user.openstackname):
            return True
        else:
            return False
