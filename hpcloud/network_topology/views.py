...
from hpcloud.network_topology.instances \
    import tables as instances_tables
from hpcloud.network_topology.ports \
    import tables as ports_tables
from hpcloud.network_topology.routers \
    import tables as routers_tables

from hpcloud.instances import\
    views as i_views
from hpcloud.instances.workflows import\
    create_instance as i_workflows
from hpcloud.networks import\
    views as n_views
from hpcloud.networks import\
    workflows as n_workflows
from hpcloud.routers import\
    views as r_views


class NTCreateRouterView (r_views.CreateView):
    template_name = 'hpcloud/network_topology/create_router.html'
    success_url = reverse_lazy("horizon:hpcloud:network_topology:index")


class NTCreateNetwork (n_workflows.CreateNetwork):
    def get_success_url(self):
        return reverse("horizon:hpcloud:network_topology:index")
...
class NTLaunchInstance (i_workflows.LaunchInstance):
    success_url = "horizon:hpcloud:network_topology:index"
...
class InstanceView (i_views.IndexView):
    ...
    template_name = 'hpcloud/network_topology/iframe.html'
...
class NetworkTopologyView (TemplateView):
    template_name = 'hpcloud/network_topology/index.html'


class JSONView(View):
    def add_resource_url(self, view, resources):
        tenant_id = self.request.user.hp_attr.tenant_id
    ...
    def get(self, request, *args, **kwargs):
        ...
        self.add_resource_url('horizon:hpcloud:instances:detail',
                              data['servers'])
        ...
        self.add_resource_url('horizon:hpcloud:networks:detail',
                              networks)
        ...
        self.add_resource_url('horizon:hpcloud:networks:ports:detail',
                              data['ports'])
        ...
        self.add_resource_url('horizon:hpcloud:routers:detail',
                              data['routers'])
