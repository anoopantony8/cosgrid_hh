...
from django.template.defaultfilters import timesince,title
from django.utils.http import urlencode
from django.utils.translation import ugettext_lazy as _
from horizon import conf,exceptions,messages,tables
from horizon.templatetags import sizeformat
from horizon.utils import filters
from hpcloud import api
from hpcloud.access_and_security.floating_ips \
    import workflows
from hpcloud.instances import tabs
...
class TerminateInstance(tables.BatchAction):
    ...
    def allowed(self, request, instance=None):
        if "Delete Instance" in request.session['user_policies'].get(request.user.hpname):
            return True
        else:
            return False
...
class LaunchLink(tables.LinkAction):
    ...
    url = "horizon:hpcloud:instances:launch"
    classes = ("btn-launch", "ajax-modal")

    def allowed(self, request, datum):
        if "Create Instance" in request.session['user_policies'].get(request.user.hpname):
            return True
        else:
            return False
...
class EditInstance(tables.LinkAction):
    ...
    url = "horizon:hpcloud:instances:update"
...
class EditInstanceSecurityGroups(EditInstance):
    ...
    def allowed(self, request, instance=None):
        return (instance.status in ACTIVE_STATES and
                ...
                request.user.hp_attr.tenant_id == instance.tenant_id)


class CreateSnapshot(tables.LinkAction):
    name = "snapshot"
    verbose_name = _("Create Snapshot")
    url = "horizon:hpcloud:images_and_snapshots:snapshots:create"
    classes = ("ajax-modal", "btn-camera")

    def allowed(self, request, instance=None):
        if "Create Snapshot" in request.session['user_policies'].get(request.user.hpname):
            return True  and instance.status in SNAPSHOT_READY_STATES \
            and not is_deleting(instance)
        else:
            return False
...
class ConsoleLink(tables.LinkAction):
    ...
    url = "horizon:hpcloud:instances:detail"
    classes = ("btn-console",)
...
class ResizeLink(tables.LinkAction):
    ...
    url = "horizon:hpcloud:instances:resize"
...
class RebuildInstance(tables.LinkAction):
    ...
    url = "horizon:hpcloud:instances:rebuild"
...
class AssociateIP(tables.LinkAction):
    ...
    url = "horizon:hpcloud:access_and_security:floating_ips:associate"
    ...
    def allowed(self, request, instance):
        if api.network.floating_ip_simple_associate_supported(request):
            ...
            if "Allocate IP" in request.session['user_policies'].get(request.user.hpname):
                return True and not is_deleting(instance)
            else:
                return False

    def get_link_url(self, datum):
        base_url = urlresolvers.reverse(self.url)
        next = urlresolvers.reverse("horizon:hpcloud:instances:index")
...
class SimpleAssociateIP(tables.Action):
    ...
    def single(self, table, request, instance_id):
        ...
        return shortcuts.redirect("horizon:hpcloud:instances:index")
...
class SimpleDisassociateIP(tables.Action):
    ...
    def allowed(self, request, instance):
        if not conf.HORIZON_CONFIG["simple_ip_management"]:
            ...
            if "Release IP" in request.session['user_policies'].get(request.user.hpname):
                return True and not is_deleting(instance)
            else:
                return False
...
class StartInstance(tables.BatchAction):
    ...
    def allowed(self, request, instance):
        if "Start Instance" in request.session['user_policies'].get(request.user.hpname):
            return True and instance.status in ("SHUTDOWN", "SHUTOFF", "CRASHED")
        else:
            return False
...
class StopInstance(tables.BatchAction):
    ...
    def allowed(self, request, instance):
        if "Stop Instance" in request.session['user_policies'].get(request.user.hpname):
            return True and ((get_power_state(instance)
...
def get_ips(instance):
    template_name = 'hpcloud/instances/_instance_ips.html'
...
class InstancesTable(tables.DataTable):
    ...
    name = tables.Column("name",
                         link=("horizon:hpcloud:instances:detail"),
