...
class DeleteGroup(tables.DeleteAction):
    ...
    def allowed(self, request, security_group=None):
        if not security_group:
            if "Delete SG" in request.session['user_policies'].get(request.user.openstackname):
                return True
            else:
                return False
...
class CreateGroup(tables.LinkAction):
    ...
    def allowed(self, request, volume=None):
        if "Create SG" in request.session['user_policies'].get(request.user.openstackname):
            return True
        else:
            return False
