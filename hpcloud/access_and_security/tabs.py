...
from horizon import exceptions,messages,tabs
from hpcloud.api import keystone,network,nova
...
class APIAccessTab(tabs.TableTab):
    ...
    def get_endpoints_data(self):
        services = []
        for i, service in enumerate(self.request.user.hp_attr.service_catalog):
            service['id'] = i
            services.append(
                keystone.Service(service, self.request.user.hp_attr.services_region))
