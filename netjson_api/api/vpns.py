...
class VPN:
    def __init__(self, id, name, host, backend, ca, cert, notes=None):
	self.id = id
        self.name = name
        self.host = host
    ...
    try:
        ...
        if resp.status_code == 200 and body:
            ...
            for vpn in vpns_list:
                vpns.append(VPN(vpn['id'], vpn['name'], vpn['host'], vpn['backend'],
...
def vpn_view(request, vpn_id):
    try:
        ...
        httpInst = httplib2.Http()
        httpInst.add_credentials(name=credential_username, password=credential_password)
        url = endpoint.strip('/') + "/vpns/%s/" % vpn_id
        resp = requests.get(url=url, auth=(credential_username, credential_password))
        LOG.debug("VPN View Status %s" % resp.status_code)
        body = resp.json()
        if resp.status_code == 200 and body:
	    print body
            return body
        ...
        logging.debug("Unable to get VPN %s" % e.message)
        return {}
