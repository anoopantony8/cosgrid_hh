...
def url_for(request, service_type, endpoint_type=None):
    ...
    catalog = request.user.hp_attr.service_catalog
    service = get_service_from_catalog(catalog, service_type)
    if service:
        url = get_url_for_service(service,
                                  request.user.hp_attr.services_region,
                                  endpoint_type)
...
def is_service_enabled(request, service_type, service_name=None):
    service = get_service_from_catalog(request.user.hp_attr.service_catalog,
                                       service_type)
    if service:
        region = request.user.hp_attr.services_region
        for endpoint in service['endpoints']:
            # ignore region for identity
...
def get_accounts(request):
    ...
    try:
        ...
        for cloud in aws_clouds:
            if cloud.name != request.user.hpname:
