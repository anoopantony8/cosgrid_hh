...
class TerminateInstance(tables.BatchAction):
    ...
    def allowed(self, request, instance=None):
        if "Delete Instance" in request.session['user_policies'].get(request.user.openstackname):
            return True
        else:
            return False
...
class LaunchLink(tables.LinkAction):
    ...
    def allowed(self, request, datum):
        try:
            ...
            if "Create Instance" in request.session['user_policies'].get(request.user.openstackname):
                return True
            else:
                return False
...
class CreateSnapshot(tables.LinkAction):
    ...
    def allowed(self, request, instance=None):
        if "Create Snapshot" in request.session['user_policies'].get(request.user.openstackname):
...
class AssociateIP(tables.LinkAction):
    ...
    def allowed(self, request, instance):
        if api.network.floating_ip_simple_associate_supported(request):
            ...
            if "Allocate IP" in request.session['user_policies'].get(request.user.openstackname):
...
class SimpleDisassociateIP(tables.Action):
    ...
    def allowed(self, request, instance):
        if not conf.HORIZON_CONFIG["simple_ip_management"]:
            ...
            if "Release IP" in request.session['user_policies'].get(request.user.openstackname):
                return True and not is_deleting(instance)
            else:
                return False
...
class StartInstance(tables.BatchAction):
    ...
    def allowed(self, request, instance):
        if "Start Instance" in request.session['user_policies'].get(request.user.openstackname):
...
class StopInstance(tables.BatchAction):
    ...
    def allowed(self, request, instance):
        if "Stop Instance" in request.session['user_policies'].get(request.user.openstackname):
